...
CREATE TABLE Names (
	...
	FISHID int --ID associated with each fish species (ex. Amphiprion percula = 1, Pterapogon kaudernii = 2)
...
CREATE TABLE SUPPLIER (
	...
	SUPPLIERID int --Unique ID # of supplier
...
CREATE TABLE SHIPMENTS (
	SHIPPINGID int --unique shipping number specifically for that shipment. Primary Key
	...
	STATUS ENUM(E, O, L) -- if the shipment came in Early (before arrival date), On time (on arrival date), or Late (delayed)
...
CREATE TABLE BOXES (
	BOXID int --unique box ID, Primary Key
	SHIPPINGID int -- ID of the shipment the box came with
	FREIGHT double(15,2) --the price it took to ship the box freight
	SHIPPING double(15, 2) --the price it took to ship the box
...
CREATE TABLE ARRIVALS (
	BAGID int -- Unique ID of the bag
	FISHID int -- ID associated with the fish species
	BASE double(15,2) -- the base price of the fish
